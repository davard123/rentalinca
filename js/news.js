(() => {
  ...
  const now=Date.now();
  rows.forEach(row=>{const label=row.querySelector('[data-age]');const age=(now-Date.parse(row.dataset.published))/86400000;if(label)label.textContent=age>30?'较早资讯':age>14?'近期发布':'近两周发布';});
